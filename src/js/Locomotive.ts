...
export class Locomotive {
    ...
    constructor(private rails: Rails, private length: number) {
    ...
    private truncateNormalizedPathLength(normalizedPathLength: number) {
        ...
        const maxProgressWithoutTrainFallingOffTheRails = 1 - this.rails.pxToNormalized(this.length);
    ...
    getPositionOnScreen(): Point[] {
        let result: Point[] = [];
        let rearWheels = this.rails.getInterpolatedTrainPosition(this.trainProgress);
        ...
        function indexOfFrontWheelsForStraightLine() {
            return this.rails.pxToNormalized(this.length) * this.rails.interpolator.length;
        }

        let i: number = indexOfFrontWheelsForStraightLine.call(this);
        while (frontWheels.distanceTo(rearWheels) < this.length) {
